...
Object.assign(ctx,{DB:{version:4,discos:[records[1]],borrados:[]},configuracionEnCurso:false,pullPromiseActual:null,pushPromiseActual:null,saveTimer:null,revisionDatos:0,leerPuntoRecuperacion:async()=>checkpoint,confirm:()=>true,crearPuntoRecuperacion:async()=>{if(fail)throw Error('quota');},normDisc:d=>({...d}),indexarFirmas:()=>{},guardarLocal:async()=>{writes++;},configurado:()=>false,renderAll:()=>{},toast:()=>{}});
await ctx.restaurarCheckpoint();assert.equal(ctx.DB.discos[0].id,'d0');assert(ctx.DB.borrados.some(x=>x.id==='d1'));assert(ctx.DB.borrados.some(x=>x.id==='old'));
